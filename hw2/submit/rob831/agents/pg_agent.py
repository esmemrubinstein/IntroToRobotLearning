...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        idxs = np.arange(0, len(rewards))
        discounted_rewards = np.power(self.gamma, idxs) * rewards
        sum_discounted_rewards = np.sum(discounted_rewards)
        discounted_returns = np.repeat(sum_discounted_rewards, len(rewards))
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for start_idx in range(len(rewards)):
            idxs = np.arange(start_idx, len(rewards))
            discounts = np.power(self.gamma, idxs-start_idx)
            discounted_rtg = discounts*rewards[start_idx:]
            sum_discounted_rtg = np.sum(discounted_rtg)
            discounted_cumsums.append(sum_discounted_rtg)
