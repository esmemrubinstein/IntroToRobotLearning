...
def _sample_worker(args):
    env_name, policy, n_steps, ep_len, seed = args
    env = gym.make(env_name)
    env.seed(seed)
    paths, envsteps = sample_trajectories(env, policy, n_steps, ep_len)
    env.close()
    return paths, envsteps

def sample_trajectories_parallel(env_name, policy, n_steps, ep_len, num_workers=4, base_seed=0):
    per_worker = n_steps // num_workers
    args = [(env_name, policy, per_worker, ep_len, base_seed + i) for i in range(num_workers)]

    with mp.Pool(processes=num_workers) as pool:
        results = pool.map(_sample_worker, args)

    # merge results
    all_paths = []
    total_envsteps = 0
    for paths, steps in results:
        all_paths.extend(paths)
        total_envsteps += steps
    return all_paths, total_envsteps
